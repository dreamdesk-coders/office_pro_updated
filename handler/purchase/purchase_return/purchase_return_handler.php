...
/* Variables for transaction log*/
	...
	$transaction = "Purchase Return Transaction";
...
if (isset($_POST['save']) || isset($_POST['update'])) {
	...
	if($currency_id == $home_currency){
		...
		$transaction_status = "PR";


if (isset($_POST['save'])) {

	/*Save to purchase return table*/

	$sql = "INSERT INTO purchase_return(invoice_id,invoice_date,staff_id,supplier_id,currency_id,office_id,exchange_rate,total_amount,grand_total_amount, refund_amount,expense,net_amount,remark,updated_date)
 	VALUES(:invoice_id,:invoice_date,:staff_id,:supplier_id, :currency_id,:office_id,:exchange_rate,:total_amount,:grand_total,:refund_amount,:expense,:net_amount,:remark,:updated_date)";
	...
	$sql = "INSERT INTO supplier_debit(invoice_id,type,supplier_id,debit_amount,currency_id)VALUES(:invoice_id,:transaction,:supplier_id,:net_amount,:currency_id)";
	$query = $dbh->prepare($sql);
	...
	/*Insert into Purchase Return Details*/
	...
	for ($x = 1; $x <= $rownum; $x++) {
		...
		if (isset($_POST[$stock_id])) {
			...
			$sql = "INSERT INTO purchase_return_details(sr_no,invoice_id,stock_id,unit_id,quantity,price,amount) VALUES(:sr_no,:invoice_id,:stock_id,:unit_id,:quantity,:price,:amount)";
...
if (isset($_POST['update'])) {
	...
	$sql = "update purchase_return set invoice_date=:invoice_date,staff_id=:staff_id,supplier_id=:supplier_id,currency_id=:currency_id,office_id=:office_id,exchange_rate=:exchange_rate,total_amount=:total_amount,grand_total_amount=:grand_total,refund_amount=:refund_amount,expense=:expense,net_amount=:net_amount,remark=:remark,updated_date=:updated_date where invoice_id=:invoice_id";
	...
	/*Supplier Debit*/
	$sql = "update supplier_debit set supplier_id=:supplier_id,debit_amount=:net_amount,currency_id=:currency_id where invoice_id=:invoice_id";
	...
	$sql="delete from purchase_return_details WHERE invoice_id=:invoice_id";
	$query = $dbh->prepare($sql);
	$query->bindParam(':invoice_id', $invoice_id, PDO::PARAM_STR);
	execute_query($dbh, $query);
...
if (isset($_GET['delete'])) {
	...
	$sql = "Select * From supplier_debit WHERE invoice_id=:invoice_id";
        $query = $dbh -> prepare($sql);
        $query->bindParam(':invoice_id',$invoice_id,PDO::PARAM_STR);
        $query->execute();
    ...
    if($debit_amount == 0){
		...
		$sql="delete from supplier_debit WHERE invoice_id=:invoice_id";
		$query = $dbh->prepare($sql);
		$query->bindParam(':invoice_id',$invoice_id,PDO::PARAM_STR);
		execute_query($dbh,$query);
		...
		$sql="delete from purchase_return WHERE invoice_id=:invoice_id";
		$query = $dbh->prepare($sql);
		$query->bindParam(':invoice_id',$invoice_id,PDO::PARAM_STR);
		execute_query($dbh,$query);
		...
		echo "<script> window.location.href = '$common_form_route?frm=purchase_return_list&del=fail'; </script>";
	}
}
	...
	switch ($action) {
		...
		case 'create':
			if (isset($_POST['invoice_print'])) {
				echo "<script type='text/javascript' language='Javascript'>setTimeout(function(){ location.replace('$common_form_route?frm=purchase_return_list')}, 100);</script>";
				echo "<script type='text/javascript' language='Javascript'>window.open('$common_form_route?frm=purchase_return_voucher&id=$transaction_id');</script>";
			} else {
				echo "<script> window.location.href = '$common_form_route?frm=purchase_return_list'; </script>";
			}
			break;
