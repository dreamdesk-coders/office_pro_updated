...
include("../../common/common_functions.php");
	...
	$transaction = "Purchase Order Transaction";
...
if(isset($_POST['save'])){
	...
	/*Save to purchase order table*/
	$status = "Active";

	$sql="INSERT INTO purchase_order(order_id,order_date,staff_id,supplier_id,currency_id,office_id,status,total_amount,tax,remark,updated_date)
 	VALUES(:order_id,:order_date,:staff_id,:supplier_id, :currency_id,:office_id,:status,:total_amount,:tax,:remark,:updated_date)";
	...
	for ($x = 1; $x <= $rownum; $x++) {
		...
		$sql="INSERT INTO purchase_order_details(sr_no,order_id,stock_id,unit_id,quantity,price,amount) VALUES(:sr_no,:order_id,:stock_id,:unit_id,:quantity,:price,:amount)";
...
if(isset($_POST['update'])) {
	...
	$sql="update purchase_order set order_date=:order_date,staff_id=:staff_id,supplier_id=:supplier_id,currency_id=:currency_id,office_id=:office_id,total_amount=:total_amount,tax=:tax,remark=:remark,updated_date=:updated_date where order_id=:order_id";
...
if (isset($_GET['delete'])) {
	...
	$sql = "Select * From purchase WHERE order_id=:order_id";
        ...
        if($len == 0){
			...
			echo "<script> window.location.href = '$common_form_route?frm=purchase_order_list&del=fail'; </script>";
        }
...
if (isset($_GET['cancel'])) {
	...
	$sql="update purchase_order set status=:status,cancel_date=:date where order_id=:order_id";
		...
		switch ($action) {
			...
			case 'create':
				if(isset($_POST['invoice_print']) ) { 
					echo "<script type='text/javascript' language='Javascript'>setTimeout(function(){ location.replace('$common_form_route?frm=purchase_order_list')}, 100);</script>";
					echo "<script type='text/javascript' language='Javascript'>window.open('$common_form_route?frm=purchase_order_voucher&id=$transaction_id');</script>";
				}else{
					echo "<script> window.location.href = '$common_form_route?frm=purchase_order_list'; </script>";
				}
				break;
			...
			case 'cancel':
				echo "<script> window.location.href = '$common_form_route?frm=purchase_order_cancel_list'; </script>";
				break;
