...
if (isset($_POST['save']) || isset($_POST['update'])) {
	...
	/* variables for deluvery transcation header */
	...
	/* variables for delivery transaction details */
...
if (isset($_POST['save'])) {
	...
	$sql = "INSERT INTO delivery(invoice_id,invoice_date,order_id,staff_id,customer_id,currency_id,office_id,exchange_rate,total_amount,grand_total_amount, paid_amount,discount,discount_percent,tax,expense,net_amount,delivery_type,remark,updated_date,status)
 	VALUES(:invoice_id,:invoice_date,:order_id,:staff_id,:customer_id, :currency_id,:office_id,:exchange_rate,:total_amount,:grand_total,:paid_amount,:discount,:discount_percent,:tax,:expense,:net_amount,:delivery_type,:remark,:updated_date,:delivery_status)";
	...
	$query->bindParam(':delivery_type', $delivery_type, PDO::PARAM_STR);
	$query->bindParam(':remark', $remark, PDO::PARAM_STR);
	$query->bindParam(':updated_date', $updated_date, PDO::PARAM_STR);
	$query->bindParam(':delivery_status', $delivery_status, PDO::PARAM_STR);
	...
	for ($x = 1; $x <= $rownum; $x++) {
		...
		if (isset($_POST[$stock_id])) {
			...
			$sql = "INSERT INTO delivery_details(sr_no,invoice_id,stock_id,unit_id,quantity,price,amount) VALUES(:sr_no,:invoice_id,:stock_id,:unit_id,:quantity,:price,:amount)";
...
if (isset($_POST['update'])) {
	...
	$sql = "update delivery set invoice_date=:invoice_date,order_id=:order_id,staff_id=:staff_id,customer_id=:customer_id,currency_id=:currency_id,office_id=:office_id,exchange_rate=:exchange_rate,total_amount=:total_amount,grand_total_amount=:grand_total,paid_amount=:paid_amount,discount=:discount,discount_percent=:discount_percent,tax=:tax,expense=:expense,net_amount=:net_amount,delivery_type=:delivery_type,remark=:remark,updated_date=:updated_date where invoice_id=:invoice_id";
	...
	$sql = "delete from delivery_details WHERE invoice_id=:invoice_id";
	$query = $dbh->prepare($sql);
	$query->bindParam(':invoice_id', $invoice_id, PDO::PARAM_STR);
	execute_query($dbh, $query);
...
if (isset($_GET['delete'])) {
	...
	if ($credit_amount == 0) {
		...
		$sql = "delete from delivery WHERE invoice_id=:invoice_id";
		$query = $dbh->prepare($sql);
		$query->bindParam(':invoice_id', $invoice_id, PDO::PARAM_STR);
		execute_query($dbh, $query);
	...
	switch ($action) {
		...
		case 'create':
			if (isset($_POST['invoice_print'])) {
				echo "<script type='text/javascript' language='Javascript'>setTimeout(function(){ location.replace('$common_form_route?frm=delivery_list')}, 100);</script>";
				echo "<script type='text/javascript' language='Javascript'>window.open('$common_form_route?frm=delivery_voucher&id=$transaction_id');</script>";
			} else {
				echo "<script> window.location.href = '$common_form_route?frm=delivery_list'; </script>";
			}
			break;
