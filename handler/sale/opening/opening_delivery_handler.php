...
if (isset($_GET['delete'])) {
	...
	$sql1 = "Select * From delivery Where invoice_id=:invoice_id";
	$query = $dbh->prepare($sql1);
	$query->bindParam(':invoice_id', $invoice_id, PDO::PARAM_STR);
	$query->execute();
	$delivery_invoice = $query->fetch(PDO::FETCH_OBJ);

	$credit_amount = $result->credit_amount;
	$delivery_type = $delivery_invoice->delivery_type;


	if ($credit_amount == 0 and $delivery_type !== "Advance") {
	...
	switch ($action) {
		...
		case 'update':
			if (isset($_POST['invoice_print'])) {
				...
				echo "<script> window.location.href = '$common_form_route?frm=opening_delivery_list'; </script>";
			}
			break;
