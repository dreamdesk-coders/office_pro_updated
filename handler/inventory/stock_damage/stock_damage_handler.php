...
include("../../common/common_functions.php");
	...
	if(isset($_POST['save'])){
			$action = "create";	
				$transaction_id = $_POST['damage_id'];
		}
	...
	$transaction = "Stock Damage Transaction";
...
if( isset($_POST['save']) || isset($_POST['update'])){
	...
	/* variables for stock_damage transcation header */ 
	...
	/* variables for stock_damage transaction details */ 
...
if(isset($_POST['save'])){
	...
	/*Save to stock_damage table*/

	$sql="INSERT INTO stock_damage(damage_id,damage_date,staff_id,currency_id,office_id,exchange_rate,total_amount,remark,updated_date)
 	VALUES(:damage_id,:damage_date,:staff_id,:currency_id,:office_id,:exchange_rate,:total_amount,:remark,:updated_date)";
	...
	/*Insert into stock_damage Details*/
	...
	for ($x = 1; $x <= $rownum; $x++) {
		...
		$sql="INSERT INTO stock_damage_details(sr_no,damage_id,stock_id,unit_id,quantity,price,amount) VALUES(:sr_no,:damage_id,:stock_id,:unit_id,:quantity,:price,:amount)";
		...
		$sql="INSERT INTO stock_balance(transaction_id,transaction_type,date,stock_id,unit_id,quantity,office_id,status) VALUES(:transaction_id,:transaction,:damage_date,:stock_id,:unit_id,:quantity,:office_id,:status)";
...
if(isset($_POST['update'])) {
	...
	$sql="update stock_damage set damage_date=:damage_date,staff_id=:staff_id,currency_id=:currency_id,office_id=:office_id,exchange_rate=:exchange_rate,total_amount=:total_amount,remark=:remark,updated_date=:updated_date where damage_id=:damage_id";
	...
	$sql="delete from stock_damage_details WHERE damage_id=:damage_id";
	$query = $dbh->prepare($sql);
	$query->bindParam(':damage_id',$damage_id,PDO::PARAM_STR);
	execute_query($dbh,$query);
...
if (isset($_GET['delete'])) {
	...
	$damage_id = $_GET['damage_id'];
        ...
        if(1==1){
			...
			$sql="delete from stock_balance WHERE transaction_id=:damage_id";
			$query = $dbh->prepare($sql);
			$query->bindParam(':damage_id',$damage_id,PDO::PARAM_STR);
			execute_query($dbh,$query);
			...
			$sql="delete from stock_damage WHERE damage_id=:damage_id";
			$query = $dbh->prepare($sql);
			$query->bindParam(':damage_id',$damage_id,PDO::PARAM_STR);
			execute_query($dbh,$query);
			...
			echo "<script> window.location.href = '$common_form_route?frm=stock_damage_list&del=fail'; </script>";
        }
		...
		switch ($action) {
			...
			case 'create':
				if(isset($_POST['invoice_print']) ) { 
					echo "<script type='text/javascript' language='Javascript'>setTimeout(function(){ location.replace('$common_form_route?frm=stock_damage_list')}, 100);</script>";
					echo "<script type='text/javascript' language='Javascript'>window.open('$common_form_route?frm=stock_damage_voucher&id=$transaction_id');</script>";
				}else{
					echo "<script> window.location.href = '$common_form_route?frm=stock_damage_list'; </script>";
				}
				break;
