...
function sum_amounts() {
    ...
    document.getElementById("total_amount").value = total_amount.toFixed(pur_dec);
    document.getElementById("grand_total").value = total_amount.toFixed(pur_dec);
    ...
    if (refund_amount == 0 || refund_amount == null) {

        refund_amount = 0;

    } else if (refund_amount < 0) {

        alert_("Info", "Refund amount cannot be less than 0.");
        localStorage.setItem("focus", "refund_amount");
        refund_amount = 0;
        $("#refund_amount").focus().val("");

    } else if (!$.isNumeric($('#refund_amount').val())) {
    ...
    grand_total = parseFloat(grand_total) + parseFloat(expense);

    // $("#total_amount").val(total_amount);
    document.getElementById("grand_total").value = grand_total.toFixed(pur_dec);

    if (refund_amount > grand_total) {

        alert_("Info", "Refund amount cannot be greater than total amount.");
        localStorage.setItem("focus", "refund_amount");
        refund_amount = 0;
        $("#refund_amount").focus().val("");
    }

    net_amount = parseFloat(grand_total) - parseFloat(refund_amount);
