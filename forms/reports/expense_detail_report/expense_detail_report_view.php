...
<div class="nav-bar no-print">
    ...
    <button id="exportExcel" class="btn btn-light" onclick="javascript:exportExcel('expense_detail_report')">Excel
        Export</button>


</div>

<div class="voucher" id="expense_detail_report">
    <div class="page-header">
        ...
        <h5 class="title">Expense Detail Report</h5><br>
        <hr>
...
$sql3 = "Select expense_date,office_id,currency_id,sum(total_amount) as total_amount from expenses where expense_date between :f_date and :t_date and expense_id between :f_expense_id and :t_expense_id and office_id between :f_office_id and :t_office_id  group by expense_date,office_id,currency_id order by expense_date asc";
    ...
    top: 10px;padding:10px !important">
                                        <td>
                                            <?php echo $num ?>
                                        </td>
                                        <td>
                ...
                echo date_format($date, "d/m/Y");?>
                                    ...
                                    <tr style="width : 100%">
                                        <thead style="background-color:#323232 !important">
                                            <tr>
                                                <th colspan="1" class="f-small">No</th>
                                                <th colspan="3" class="f-small">Expense ID</th>
                                                <th colspan="3" class="f-small">Description</th>
                                                <th colspan="3" class="f-small">Amount</th>
                                            </tr>
                                        </thead>
                                        <?php

                $sql_emaster = "Select * from expenses_details where expense_id = :f_expense_id_ order by expense_id asc";
                $query_emaster = $dbh->prepare($sql_emaster);

                $query_emaster->bindParam(':f_expense_id_', $result->expense_id, PDO::PARAM_STR);

                $query_emaster->execute();
                $results_emaster = $query_emaster->fetchAll(PDO::FETCH_OBJ);
                $num_emaster = 1;

                foreach ($results_emaster as $result_) {
                    ?>
                                    <tr>
                                        <td colspan="1" class="f-small border_">
                                            <?php echo $num_emaster ?>
                                        </td>
                                        <td colspan="3" class="f-small border_">
                                            <?php echo htmlentities($result_->exp_master_id); ?>
                                        </td>
                                        <td colspan="3" class="f-small border_">
                                            <?php echo htmlentities($result_->exp_master_id); ?>
                                        </td>
                                        <td colspan="3" class="f-small border_">
                                            <?php echo htmlentities($result_->amount); ?>
                                        </td>
                                    </tr>
