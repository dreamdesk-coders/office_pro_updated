...
if (strlen($_SESSION['alogin']) == 0) {
    ...
    if (isset($_GET['f_office_id']) && isset($_GET['t_office_id'])) {
        $f_date = $_GET['f_date'];
        $t_date = $_GET['t_date'];
        $f_use_id = $_GET['f_use_id'];
        $t_use_id = $_GET['t_use_id'];
...
?>
    ...
    <div class="nav-bar no-print">
        ...
        <button id="exportExcel" class="btn btn-light" onclick="javascript:exportExcel('office_use_summary_report')">Excel Export</button>
    </div>

    <table id="office_use_summary_report" class="voucher">
        <tr>
            <td>
                <div class="heading">
                ...
                <br>
                <h5 class="title">Office Use Summary Report</h5>
                <hr><br>

                <?php


                $sql3 = "Select use_date,$group_by,sum(total_amount) as grand_total_amount From office_use where use_date between :f_date and :t_date and use_id between :f_use_id and :t_use_id and currency_id between :f_currency_id and :t_currency_id and staff_id between :f_staff_id and :t_staff_id and office_id between :f_office_id and :t_office_id  group by use_date,$group_by order by use_date asc";

                $query3 = $dbh->prepare($sql3);

                $query3->bindParam(':f_date', $f_date, PDO::PARAM_STR);
                $query3->bindParam(':t_date', $t_date, PDO::PARAM_STR);
                $query3->bindParam(':f_use_id', $f_use_id, PDO::PARAM_STR);
                $query3->bindParam(':t_use_id', $t_use_id, PDO::PARAM_STR);
                ...
                ?>
                        ...
                        <table class="print-content table table-bordered">
                            <h6 class="groupby">
                                ...
                                <b> Office Use Date : <?php $date = date_create($result3->use_date);
                                ...
                                <thead>
                                    <tr>
                                        <th>No</th>
                                        <th style="width: 100px;">Office Use ID</th>
                                ...
                                <tbody>
                                    ...
                                    $sql = "Select * From office_use where use_date between :f_date and :t_date and use_id between :f_use_id and :t_use_id and currency_id between :f_currency_id and :t_currency_id and staff_id between :f_staff_id and :t_staff_id and office_id between :f_office_id and :t_office_id and use_date = :group_use_date and currency_id = :group_currency_id order by use_date asc";

                                    $query = $dbh->prepare($sql);

                                    $query->bindParam(':f_date', $f_date, PDO::PARAM_STR);
                                    $query->bindParam(':t_date', $t_date, PDO::PARAM_STR);
                                    $query->bindParam(':f_use_id', $f_use_id, PDO::PARAM_STR);
                                    $query->bindParam(':t_use_id', $t_use_id, PDO::PARAM_STR);
                                    ...
                                    $query->bindParam(':group_use_date', $result3->use_date, PDO::PARAM_STR);
                                    ...
                                    ?>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <?php echo htmlentities($result->use_id); ?>
                                            </td>
                                            <td>
                                                <?php $date = date_create($result->use_date);
                                                echo date_format($date, "d/m/Y"); ?>
                                            </td>
                                            <td>
