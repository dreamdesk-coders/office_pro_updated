...
if (strlen($_SESSION['alogin']) == 0) {
    ...
    if (isset($_GET['f_office_id']) && isset($_GET['t_office_id'])) {
        $f_date = $_GET['f_date'];
        $t_date = $_GET['t_date'];
        $f_invoice_id_sale = $_GET['f_invoice_id_sale'];
        $t_invoice_id_sale = $_GET['t_invoice_id_sale'];
        $f_invoice_id_delivery = $_GET['f_invoice_id_delivery'];
        $t_invoice_id_delivery = $_GET['t_invoice_id_delivery'];
...
?>
    ...
    <div class="nav-bar no-print">
        ...
        <button id="exportExcel" class="btn btn-light" onclick="javascript:exportExcel('sale_summary_report')">Excel Export</button>
    </div>

    <table id="sale_summary_report" class="voucher">
        <tr>
            <td>
                <div class="heading">
                ...
                <br>
                <h5 class="title">Sale Summary Report</h5>
                <hr><br>
                ...
                if ($sale_type == "all") {
                    $sql3 = "Select invoice_date,$group_by,sum(net_amount) as net_total_amount , sum(grand_total_amount) as grand_total_amount from (
                    (Select * From sale where invoice_date between :f_date and :t_date and invoice_id between :f_invoice_id_sale and :t_invoice_id_sale and customer_id between :f_customer_id and :t_customer_id and currency_id between :f_currency_id and :t_currency_id and staff_id between :f_staff_id and :t_staff_id and office_id between :f_office_id and :t_office_id)
                    UNION ALL 
                    (Select * From delivery where invoice_date between :f_date and :t_date and invoice_id between :f_invoice_id_delivery and :t_invoice_id_delivery and customer_id between :f_customer_id and :t_customer_id and currency_id between :f_currency_id and :t_currency_id and staff_id between :f_staff_id and :t_staff_id and office_id between :f_office_id and :t_office_id)
                     order by invoice_date asc )t1 group by invoice_date,$group_by";
                } else {
                    $sql3 = "Select invoice_date,$group_by,sum(net_amount) as net_total_amount , sum(grand_total_amount) as grand_total_amount from (
                        (Select * From sale where invoice_date between :f_date and :t_date and invoice_id between :f_invoice_id_sale and :t_invoice_id_sale and customer_id between :f_customer_id and :t_customer_id and currency_id between :f_currency_id and :t_currency_id and staff_id between :f_staff_id and :t_staff_id and office_id between :f_office_id and :t_office_id and sale_type = :sale_type)
                        UNION ALL 
                        (Select * From delivery where invoice_date between :f_date and :t_date and invoice_id between :f_invoice_id_delivery and :t_invoice_id_delivery and customer_id between :f_customer_id and :t_customer_id and currency_id between :f_currency_id and :t_currency_id and staff_id between :f_staff_id and :t_staff_id and office_id between :f_office_id and :t_office_id  and delivery_type = :sale_type)
                         order by invoice_date asc )t1 group by invoice_date,$group_by";
                }
                ...
                $query3->bindParam(':f_invoice_id_sale', $f_invoice_id_sale, PDO::PARAM_STR);
                $query3->bindParam(':t_invoice_id_sale', $t_invoice_id_sale, PDO::PARAM_STR);
                $query3->bindParam(':f_invoice_id_delivery', $f_invoice_id_delivery, PDO::PARAM_STR);
                $query3->bindParam(':t_invoice_id_delivery', $t_invoice_id_delivery, PDO::PARAM_STR);
                        ...
                        <table class="print-content table table-bordered">
                            <h6 class="groupby">
                                ...
                                <tbody>
                                    ...
                                    if ($sale_type == "all") {
                                        $sql = "Select * from (
                                        (Select * From sale where invoice_date between :f_date and :t_date and invoice_id between :f_invoice_id_sale and :t_invoice_id_sale and customer_id between :f_customer_id and :t_customer_id and currency_id between :f_currency_id and :t_currency_id and staff_id between :f_staff_id and :t_staff_id and office_id between :f_office_id and :t_office_id and invoice_date = :group_invoice_date and $group_by = :group_by_id)
                                        UNION ALL
                                        (Select * From delivery where invoice_date between :f_date and :t_date and invoice_id between :f_invoice_id_delivery and :t_invoice_id_delivery and customer_id between :f_customer_id and :t_customer_id and currency_id between :f_currency_id and :t_currency_id and staff_id between :f_staff_id and :t_staff_id and office_id between :f_office_id and :t_office_id and invoice_date = :group_invoice_date and $group_by = :group_by_id)
                                        order by invoice_date asc)t2";
                                    } else {
                                        $sql = "Select * from (
                                            (Select * From sale where invoice_date between :f_date and :t_date and invoice_id between :f_invoice_id_sale and :t_invoice_id_sale and customer_id between :f_customer_id and :t_customer_id and currency_id between :f_currency_id and :t_currency_id and staff_id between :f_staff_id and :t_staff_id and office_id between :f_office_id and :t_office_id and invoice_date = :group_invoice_date and $group_by = :group_by_id and sale_type = :sale_type)
                                            UNION ALL
                                            (Select * From delivery where invoice_date between :f_date and :t_date and invoice_id between :f_invoice_id_delivery and :t_invoice_id_delivery and customer_id between :f_customer_id and :t_customer_id and currency_id between :f_currency_id and :t_currency_id and staff_id between :f_staff_id and :t_staff_id and office_id between :f_office_id and :t_office_id and invoice_date = :group_invoice_date and $group_by = :group_by_id and delivery_type = :sale_type)
                                            order by invoice_date asc)t2";
                                    }
                                    ...
                                    $query->bindParam(':f_invoice_id_sale', $f_invoice_id_sale, PDO::PARAM_STR);
                                    $query->bindParam(':t_invoice_id_sale', $t_invoice_id_sale, PDO::PARAM_STR);
                                    $query->bindParam(':f_invoice_id_delivery', $f_invoice_id_delivery, PDO::PARAM_STR);
                                    $query->bindParam(':t_invoice_id_delivery', $t_invoice_id_delivery, PDO::PARAM_STR);
