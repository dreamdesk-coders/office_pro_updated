...
if (strlen($_SESSION['alogin']) == 0) {
    ...
    if (isset($_GET['f_office_id']) && isset($_GET['t_office_id'])) {
        $f_date = $_GET['f_date'];
        $t_date = $_GET['t_date'];
        $f_transfer_id = $_GET['f_transfer_id'];
        $t_transfer_id = $_GET['t_transfer_id'];
...
?>
    ...
    <div class="nav-bar no-print">
        ...
        <button id="exportExcel" class="btn btn-light" onclick="javascript:exportExcel('stock_transfer_summary_report')">Excel Export</button>
    </div>

    <table id="stock_transfer_summary_report" class="voucher">
        <tr>
            <td>
                <div class="heading">
                ...
                <br>
                <h5 class="title">Stock Transfer Summary Report</h5>
                <hr><br>

                <?php


                $sql3 = "Select transfer_date,$group_by,sum(total_amount) as grand_total_amount From stock_transfer where transfer_date between :f_date and :t_date and transfer_id between :f_transfer_id and :t_transfer_id and currency_id between :f_currency_id and :t_currency_id and staff_id between :f_staff_id and :t_staff_id and office_id between :f_office_id and :t_office_id  group by transfer_date,$group_by order by transfer_date asc";

                $query3 = $dbh->prepare($sql3);

                $query3->bindParam(':f_date', $f_date, PDO::PARAM_STR);
                $query3->bindParam(':t_date', $t_date, PDO::PARAM_STR);
                $query3->bindParam(':f_transfer_id', $f_transfer_id, PDO::PARAM_STR);
                $query3->bindParam(':t_transfer_id', $t_transfer_id, PDO::PARAM_STR);
                ...
                ?>
                        ...
                        <table class="print-content table table-bordered">
                            <h6 class="groupby">
                                ...
                                <b> Transfer Date : <?php $date = date_create($result3->transfer_date);
                                ...
                                <tbody>
                                    ...
                                    $sql = "Select * From stock_transfer where transfer_date between :f_date and :t_date and transfer_id between :f_transfer_id and :t_transfer_id and currency_id between :f_currency_id and :t_currency_id and staff_id between :f_staff_id and :t_staff_id and office_id between :f_office_id and :t_office_id and transfer_date = :group_transfer_date and currency_id = :group_currency_id order by transfer_date asc";

                                    $query = $dbh->prepare($sql);

                                    $query->bindParam(':f_date', $f_date, PDO::PARAM_STR);
                                    $query->bindParam(':t_date', $t_date, PDO::PARAM_STR);
                                    $query->bindParam(':f_transfer_id', $f_transfer_id, PDO::PARAM_STR);
                                    $query->bindParam(':t_transfer_id', $t_transfer_id, PDO::PARAM_STR);
                                    ...
                                    $query->bindParam(':group_transfer_date', $result3->transfer_date, PDO::PARAM_STR);
                                    ...
                                    ?>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <?php echo htmlentities($result->transfer_id); ?>
                                            </td>
                                            <td>
                                                <?php $date = date_create($result->transfer_date);
                                                echo date_format($date, "d/m/Y"); ?>
                                            </td>
                                            <td>
                                            ...
                                            <td>
                                                <?php echo htmlentities($result->to_office_id); ?>
                                            </td>
                                            <td>
