...
if(isset($_SESSION['save_status'])){
	...
	if($save_status == "1"){
		if($action == "create" or $action == "update"){
			echo "<script> alert_('Info','Office Use ID : $transaction_id is successfully saved.');</script>";
		}
		elseif($action == "delete"){
			echo "<script> alert_('Info','Office Use ID : $transaction_id is successfully deleted.');</script>";
		}
	}
...
if($role == "Management Admin"){
	$sql = "Select * From office_use order by use_date desc";
	$query = $dbh -> prepare($sql);
}
else{
	$office_id = $_SESSION['office_id'];
	$sql = "Select * From office_use where office_id=:office_id order by use_date desc";
	$query = $dbh -> prepare($sql);
...
<div class="main-content">
	...
	<div class="container">
		<div class="row">
			<div class="col-md-12">
                  <a href="<?php echo $common_form_route."?frm=office_use_add" ?>" class="btn btn-primary">Add New Office Use</a>
				<br><br>
			</div>
		</div>
		<div class="row">
			<div class="col-md-12" style="background-color: #fff;">
				<br>
				<h4>Office Use Transactions</h4>
				<br>
				<div class="res-table">
				<table class="table table-striped" id="office-use-table">
				  <thead>
				    <tr>
				      <td scope="col">No</td>
				      <td scope="col">Office Use ID</td>
				  ...
				  <tbody>
					...
					<tr>
						...
						<td><?php echo $use_id; ?></td>
						<td><?php echo $use_date; ?></td>
						<td><?php echo $office_id; ?></td>
						<td><?php echo $staff_id; ?></td>
						<td><?php echo $currency_id; ?></td>
						<td><?php echo $total_amount; ?></td>
						<td><a href="<?php echo $common_form_route."?frm=office_use_update&id=".$use_id ?>" class="btn btn-info">View</a></td>
					</tr>
