...
if(isset($_SESSION['save_status'])){
	...
	elseif($save_status == "0"){
		...
		elseif($action == "delete"){
			echo "<script> alert_('Info','Cannot delete transaction.');</script>";
		}
	}
...
if($role == "Management Admin"){
	$sql = "Select * From sale_return order by invoice_date desc";
	$query = $dbh -> prepare($sql);
}
else{
	$office_id = $_SESSION['office_id'];
	$sql = "Select * From sale_return where office_id =:office_id order by invoice_date desc";
	$query = $dbh -> prepare($sql);
...
<div class="main-content">
	...
	<div class="container">
		<div class="row">
			<div class="col-md-12">
                  <a href="<?php echo $common_form_route."?frm=sale_return_add" ?>" class="btn btn-primary">Add New Sale Return</a>
				<br><br>
			</div>
		</div>
		<div class="row">
			<div class="col-md-12" style="background-color: #fff;">
				<br>
				<h4>Sale Return Transactions</h4>
				<br>
				<div class="res-table">
				<table class="table table-striped" id="sale-return-table">
				  <thead>
				    <tr>
				      <td scope="col">No</td>
				  ...
				  <tbody>
					...
					<tr>
						...
						<td><a href="<?php echo $common_form_route."?frm=sale_return_update&id=".$invoice_id ?>" class="btn btn-info">View</a></td>
