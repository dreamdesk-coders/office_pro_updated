...
if($role == "Management Admin"){
    switch ($delivery_type_) {
		case 'all':
			$sql = "Select * From delivery where status = 'new' and invoice_date between :from and :to order by invoice_date desc";
			break;

		case 'advance':
			$sql = "Select * From delivery where delivery_type ='advance' and status = 'new' and invoice_date between :from and :to order by invoice_date desc";
			break;
		
		case 'cash':
			$sql = "Select * From delivery where delivery_type = 'cash' and status = 'new' and invoice_date between :from and :to order by invoice_date desc";
			break;

		case 'credit':
			$sql = "Select * From delivery where delivery_type = 'credit' and status = 'new' and invoice_date between :from and :to order by invoice_date desc";
			break;
	...
	switch ($delivery_type_) {
		case 'all':
			$sql = "Select * From delivery where office_id =:office_id and status = 'new' and invoice_date between :from and :to order by invoice_date desc";
			break;

		case 'advance':
			$sql = "Select * From delivery where delivery_type ='advance' and office_id =:office_id and status = 'new' and invoice_date between :from and :to order by invoice_date desc";
			break;
		
		case 'cash':
			$sql = "Select * From delivery where delivery_type = 'cash' and office_id =:office_id and status = 'new' and invoice_date between :from and :to order by invoice_date desc";
			break;

		case 'credit':
			$sql = "Select * From delivery where delivery_type = 'credit' and office_id =:office_id and status = 'new' and invoice_date between :from and :to order by invoice_date desc";
			break;
            ...
            <div class="col-md-12">
            	<div class="d-flex flex-row pb-0 mb-0">
                    ...
                    <select class="form-control w-25  ml-2" id='selectBox'>
						...
						<option value='?frm=delivery_list&delivery_type=all' <?php if($delivery_type_ === "all"){echo 'selected';} ?>>All</option>
						<option value='?frm=delivery_list&delivery_type=advance' <?php if($delivery_type_ === "advance"){echo 'selected';} ?>>Advance</option>
						<option value='?frm=delivery_list&delivery_type=cash' <?php if($delivery_type_ === "cash"){echo 'selected';} ?>>Cash</option>
						<option value='?frm=delivery_list&delivery_type=credit' <?php if($delivery_type_ === "credit"){echo 'selected';} ?>>Credit</option>
					</select>
                    ...
                    <tbody>
				  	...
				  	foreach($results as $result){
                        ...
                        <tr>
                            ...
                            <td><a href="<?php echo $common_form_route."?frm=do_delivery_update&id=".$invoice_id ?>"
                                    class="btn btn-info">View</a></td>
                            <?php }else{?>

                            <td><a href="<?php echo $common_form_route."?frm=delivery_update&id=".$invoice_id ?>"
                                    class="btn btn-info">View</a></td>
