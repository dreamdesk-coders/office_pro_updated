...
if($role == "Management Admin"){
	switch ($sale_type_) {
		case 'all':
			$sql = "Select * From sale where status = 'new' order by invoice_date desc";
			break;

		case 'advance':
			$sql = "Select * From sale where sale_type ='advance' and status = 'new' order by invoice_date desc";
			break;
		
		case 'cash':
			$sql = "Select * From sale where sale_type = 'cash' and status = 'new' order by invoice_date desc";
			break;

		case 'credit':
			$sql = "Select * From sale where sale_type = 'credit' and status = 'new' order by invoice_date desc";
			break;
	...
	switch ($sale_type_) {
		case 'all':
			$sql = "Select * From sale where office_id =:office_id and status = 'new' order by invoice_date desc";
			break;

		case 'advance':
			$sql = "Select * From sale where sale_type ='advance' and office_id =:office_id and status = 'new' order by invoice_date desc";
			break;
		
		case 'cash':
			$sql = "Select * From sale where sale_type = 'cash' and office_id =:office_id and status = 'new' order by invoice_date desc";
			break;

		case 'credit':
			$sql = "Select * From sale where sale_type = 'credit' and office_id =:office_id and status = 'new' order by invoice_date desc";
			break;
		...
		<div class="row">
			<div class="col-md-12">
				<div class="d-flex flex-row pb-0 mb-0">
					<a href="<?php echo $common_form_route."?frm=sale_add" ?>" class="btn btn-primary">Add New Sale</a>
					<select class="form-control w-25  ml-2" onchange="filter_sales_list()" id='selectBox'>
						<option disabled>Filter</option>
						<option value='?frm=sale_list&sale_type=all' <?php if($sale_type_ === "all"){echo 'selected';} ?>>All</option>
						<option value='?frm=sale_list&sale_type=advance' <?php if($sale_type_ === "advance"){echo 'selected';} ?>>Advance</option>
						<option value='?frm=sale_list&sale_type=cash' <?php if($sale_type_ === "cash"){echo 'selected';} ?>>Cash</option>
						<option value='?frm=sale_list&sale_type=credit' <?php if($sale_type_ === "credit"){echo 'selected';} ?>>Credit</option>
					</select>
				</div>
				<br>
			</div>
				...
				<table class="table table-striped" id="sale-table">
				  ...
				  <tbody>
					...
					<tr>
						...
						<td><a href="<?php echo $common_form_route."?frm=so_sale_update&id=".$invoice_id ?>" class="btn btn-info">View</a></td>
						<?php }else{?>

						<td><a href="<?php echo $common_form_route."?frm=sale_update&id=".$invoice_id ?>" class="btn btn-info">View</a></td>
